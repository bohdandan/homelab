...
class KioskConfigurationTest(unittest.TestCase):
    def test_kiosk_source_tree_exists(self) -> None:
        ...
        self.assertTrue((app_root / "config/routine.json").is_file())
        self.assertTrue((app_root / "config/major-events.json").is_file())
        self.assertTrue((app_root / "config/chinese.json").is_file())
    ...
    def test_kiosk_repo_wiring_exists(self) -> None:
        ...
        self.assertIn("config/routine.json", manifest)
        self.assertIn("config/major-events.json", manifest)
        self.assertIn("config/chinese.json", manifest)
        self.assertIn("mountPath: /usr/share/nginx/html/config/routine.json", manifest)
        self.assertIn("mountPath: /usr/share/nginx/html/config/major-events.json", manifest)
        self.assertIn("mountPath: /usr/share/nginx/html/config/chinese.json", manifest)
